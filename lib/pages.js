...
const WKND_PREFIX = '/content/wknd/us/en';
const FETCH_CONFIG = {
  headers: {
...
export async function getPageModelWithFallback(path, fallbackPath) {
  ...
  let pageModel$ = getPageModel(path);
  let fallback$ = getPageModel(fallbackPath);
  let pageModel = await pageModel$;
  
  if (pageModel == null) {
      pageModel = await fallback$;
  }
...
export async function getChildrenPaths(path, staticPaths) {
  ...
  const promises = children.map(async childPath => {
    const spaPath = childPath.replace(WKND_PREFIX, '')
